...
$if      set lbdcon     $if     set normal      1 / nyrs(t) * (1 + tk) * dfact(t) * sum(new(i)$( i_lea(i)),                          sum(tv(t,v)$ivt(i,v,t),  CAPEXEUR_CON(i,v)        *  endeffecteur(i,v,t))) +
$if      set lbdcon     $if     set mixed                                           sum(new(i)$( i_lea(i)), sum(inv, rshare(inv,i) * sum(tv(t,v)$ivt(i,v,t),  CAPEXEUR_CON(i,v)        * rzeta(inv,i,v))))
$if      set lbdnlp     $if     set normal      1 / nyrs(t) * (1 + tk) * dfact(t) * sum(new(i)$( i_lea(i)),                          sum(tv(t,v)$ivt(i,v,t),  CAPEXEUR_NLP(i,v)        *  endeffecteur(i,v,t))) +
$if      set lbdnlp     $if     set mixed                                           sum(new(i)$( i_lea(i)), sum(inv, rshare(inv,i) * sum(tv(t,v)$ivt(i,v,t),  CAPEXEUR_NLP(i,v)        * rzeta(inv,i,v))))
...
$if     set lbs                                     IX.FX(i,r,"2022")$(not gas(i)) = 0 ;  
$if     set onlygas2022                             IX.FX(i,r,"2022")$(not gas(i)) = 0 ;
