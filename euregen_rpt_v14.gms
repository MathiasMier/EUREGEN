...
discost_rpt(t,r,tyrpt)$(sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t)))) > 0)  = sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), discost(i,v,r,t) * XTWH.L(i,v,r,t))))
                                                                                                          / sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t)))) + eps ;

discost_total_rpt(t,tyrpt)$(sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t))))) > 0)  = sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), discost(i,v,r,t) * XTWH.L(i,v,r,t)))))
                                                                                                                      / sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t))))) + eps ;
                                                                                                                      
discostco2_rpt(t,r,tyrpt)$(sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t)))) > 0)  = sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), discostco2(i,v,r,t) * XTWH.L(i,v,r,t))))
                                                                                                          / sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t)))) + eps ;

discostco2_total_rpt(t,tyrpt)$(sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t))))) > 0)  = sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), discostco2(i,v,r,t) * XTWH.L(i,v,r,t)))))
                                                                                                                      / sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t))))) + eps ;

                                                                                                                      
fomcost_rpt(t,r,tyrpt)$(sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t)))) > 0)  = sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), fomcost(i,v,r) * XTWH.L(i,v,r,t))))
                                                                                                          / sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t)))) + eps ;

fomcost_total_rpt(t,tyrpt)$(sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t))))) > 0)  = sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), fomcost(i,v,r) * XTWH.L(i,v,r,t)))))
                                                                                                                      / sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t))))) + eps ;                                                                                                                        

effrate_rpt(t,r,tyrpt)$(sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t)))) > 0)  = sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), effrate(i,v,r) * XTWH.L(i,v,r,t))))
                                                                                                          / sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t)))) + eps ;

effrate_total_rpt(t,tyrpt)$(sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t))))) > 0)  = sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), effrate(i,v,r) * XTWH.L(i,v,r,t)))))
                                                                                                                      / sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t))))) + eps ;  

emit_rpt(t,r,tyrpt)$(sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t)))) > 0)  = sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), emit(i,v,r) * XTWH.L(i,v,r,t))))
                                                                                                          / sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t)))) + eps ;

emit_total_rpt(t,tyrpt)$(sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t))))) > 0)  = sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), emit(i,v,r) * XTWH.L(i,v,r,t)))))
                                                                                                                      / sum(r, sum(xtyperpt(tyrpt,type), sum(idef(i,type), sum(tv(t,v), XTWH.L(i,v,r,t))))) + eps ;  
...
execute_unload'report_verysimple_%m%\%e%_rpt.gdx',               Emissions_total_rpt,
                                                                 ...
                                                                 Electricity1_rpt,
                                                                 Electricity1_GER_rpt,
                                                                 Electricity1_FR_rpt,
                                                                 Electricity2_total_rpt,
                                                                 Electricity2_rpt,
                                                                 Electricity2_GER_rpt,
                                                                 Electricity2_FR_rpt,
                                                                 Electricity3_total_rpt,
                                                                 Electricity3_rpt,
                                                                 Electricity3_GER_rpt,
                                                                 Electricity3_FR_rpt,
                                                                 ElectricityGeneration_total_xtype_rpt,
                                                                 ElectricityGeneration_xtype_rpt,
                                                                 ElectricityGeneration_GER_xtype_rpt,
                                                                 ElectricityGeneration_FR_xtype_rpt,
                                                                 InstalledCapacities_total_xtype_rpt,
                                                                 InstalledCapacities_xtype_rpt,
                                                                 AddedCapacities_total_xtype_rpt,
                                                                 AddedCapacities_xtype_rpt,
                                                                 Storage_All_rpt,
                                                                 NTCTransfers_total_rpt,
                                                                 gasuse, gasuseeu, noweffect_rpt, effect_rpt,
                                                                 discost_rpt, discost_total_rpt,discostco2_rpt,discostco2_total_rpt,fomcost_rpt,fomcost_total_rpt,capcost_rpt,capcost_total_rpt,effrate_rpt,effrate_total_rpt,emit_rpt,emit_total_rpt
                                                                 gcapcost_rpt,gcapcost_total_rpt,gfomcost_rpt,gfomcost_total_rpt,geffrate_rpt,geffrate_total_rpt,
                                                                 tcapcost_rpt,tcapcost_total_rpt,tfomcost_rpt,tfomcost_total_rpt,teffrate_rpt,teffrate_total_rpt
                                                                 ;
