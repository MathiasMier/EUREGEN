...
$if      set lbseur      $if    set normal                          1 / nyrs(t) * (1 + tk) * dfact(t) * sum(new(i)$(ki_lea(i)), sum(tv(t,v)$ivt(i,v,t), KCAPEXEUR_MIP(i,t)        * kendeffecteur(i,t))) +
$if      set lbseur      $if    set mixed                           1 / nyrs(t) * (1 + tk) * dfact(t) * sum(new(i)$(ki_lea(i)), sum(tv(t,v)$ivt(i,v,t), KCAPEXEUR_MIP(i,t)        * kendeffecteur(i,t))) +
...
$if      set lbs         $if    set mixed                         + sum(new(i)$(kir_lea(i,r)), sum(inv, share(inv,i,r) * sum(tv(t,v)$ivrt(i,v,r,t), KCAPEX_MIP(i,v,r) * 1e+3 * zeta(inv,i,v,r))))
               )
