...
eq_rd_budget_it(i,t)$(ki_lea(i) and tmyopic(t))..
...
convtarget(i,r,t)$(sameas(i,"Nuclear") and sameas(r,"France") and tmyopic(t))..
    sum(v, XTWH(i,v,r,t)) =g= 0.75 * daref(r,t) ;
...
$if set uselimits       $if     set banking $if not set frnuctgt    $gdxin limits\%l%_bauprice.gdx
$if set uselimits       $if not set banking $if not set frnuctgt    $gdxin limits\%l%_bauprice_nobanking.gdx
$if set uselimits       $if     set banking $if     set frnuctgt    $gdxin limits\%l%_bauprice_frnuctgt.gdx
$if set uselimits       $if not set banking $if     set frnuctgt    $gdxin limits\%l%_bauprice_frnuctgt_nobanking.gdx
